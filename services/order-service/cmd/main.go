...
func main() {
	...
	log.Info("Starting Order Service...", logger.Fields{"version": version})
	...
	if redisHost != "" {
		...
		if err := redisClient.Ping(ctx).Err(); err != nil {
			...
			rateLimiter = ratelimit.NewRedisLimiter(redisClient, 100, time.Minute, "order")
		}
	} else {
	...
	protected.Use(auth.AuthMiddleware(jwtManager))
	{
		orders := protected.Group("/orders")
		{
			...
			orders.GET("",
				...
				auth.RequireRole(auth.RoleUser, auth.RoleManager, auth.RoleAdmin),
				httpHandler.ListOrders,
			)

			// Update order - managers and admins only
			orders.PUT("/:id",
				auth.RequireRole(auth.RoleManager, auth.RoleAdmin),
				httpHandler.UpdateOrder,
			)
	...
	log.Info("Order Service ready", logger.Fields{
		"http_port": httpPort,
		"version":   version,
	})
	...
	log.Info("Order Service stopped gracefully", nil)
}
